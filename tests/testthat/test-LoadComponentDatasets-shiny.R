test_that("LoadComponentDatasets module works", {
  withr::local_envvar(list(DOMINO_DATASETS_DIR = system.file("tests_data", "load_data", package = "assistDomino")))
  app <- shinyApp(
    fluidPage(assistDomino:::LoadComponentDatasets$shiny$ui("test")),
    function(input, output, session) {
      res <- assistDomino:::LoadComponentDatasets$shiny$server("test")
      exportTestValues(
        name = res$name(),
  ...
  driver <- shinytest2::AppDriver$new(app, expect_values_screenshot_args = FALSE, name = "LoadComponentDatasets")

  driver$set_inputs(
    `test-filebrowser-file_clicked` = file.path(get_user_datasets_dir(), "cars.csv"),
  ...
  driver$set_inputs(
    `test-filebrowser-file_clicked` = file.path(get_user_datasets_dir(), "cars.xlsx"),
    allow_no_input_binding_ = TRUE, priority_ = "event"
  )
  driver$wait_for_idle()
  driver$expect_values(export = TRUE)
  ...
  driver$set_inputs(
    `test-filebrowser-file_clicked` = file.path(get_user_datasets_dir(), "cars_wrong_format.sas7bdat"),
    allow_no_input_binding_ = TRUE, priority_ = "event"
  )
  driver$wait_for_idle()
  ...
  driver$set_inputs(
    `test-filebrowser-file_clicked` = file.path(get_user_datasets_dir(), "cars_semicolon.txt"),
    allow_no_input_binding_ = TRUE, priority_ = "event"
  )
  driver$wait_for_idle()
  driver$expect_values(export = TRUE)
  ...
  driver$set_inputs(
    `test-filebrowser-file_clicked` = file.path(get_user_datasets_dir(), "cars.sav"),
    allow_no_input_binding_ = TRUE, priority_ = "event"
  )
  driver$wait_for_idle()
  driver$expect_values(export = TRUE)
