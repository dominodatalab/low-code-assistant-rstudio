...
#' @param data_name The name of a data frame in the global environment. Can be reactive or static.
#' @export
...
#' @param data_name The name of a data frame in the global environment. Can be reactive or static.
#' @export
assist_viz <- function(data_name = NULL) {
  app <- shiny::shinyApp(
    ui = shiny::fluidPage(page_viz_ui("app")),
    server = function(input, output, session) page_viz_server("app", data_name_in = data_name)
  )
  shiny::runGadget(app, viewer = shiny::dialogViewer("Domino R Assistant", 1100, 800))
}
...
#' Low Code Assistant - Visualizations
#'
#' Run the data visualizations LCA wizard. If a variable holding a data frame is selected
#' when invoking the addin in RStudio, that data frame will be used.
#' @export
assist_viz_addin <- function() {
  ...
  assist_viz(data_name = data_name)
}
